...
def split_sizes():
    for dataset in 'rest', 'device', 'lap':
        ...
        print(f"\n\n{dataset}")
        dataset = load_dataset("json", data_files=f"{ds_path}.json")["train"]
...
def load_data(dataset, pos_ex_only=False):
    ...
    d_label = load_dataset("json", data_files=f"{ds_path}_label.json")["train"]
    if pos_ex_only:
        d_label = d_label.filter(lambda ex: 'B-ASP' in ex['tags'])

    d_test = load_dataset("json", data_files=f"{ds_path}_test.json")["train"]
    d_unlabeled = load_dataset("json", data_files=f"{ds_path}_unlabeled.json")["train"]
...
class TuneFewShot:
    ...
    def split_into_label_and_test(self, dataset: str, max_num_label: int=1000):
        ...
        if not os.path.exists(label_path) or not os.path.exists(test_path):
            ...
            d_label = load_dataset("json", data_files=label_path)["train"]
            d_test = load_dataset("json", data_files=test_path)["train"]
            d_unlabeled = load_dataset("json", data_files=unlabeled_path)["train"]
        return d_label, d_test, d_unlabeled
